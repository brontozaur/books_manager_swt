...
public class ExportXlsOptionsView extends AbstractExportView {
	...
	private class ExportSettings extends AbstractIConfigAdapter {
		...
		private Button buttonAutoAlignCols;
		...
		public final void createContents() {
			...
			this.buttonAutoAlignCols = new Button(groupOptions, SWT.CHECK);
			GridDataFactory.fillDefaults().align(SWT.BEGINNING, SWT.BEGINNING).span(((GridLayout) groupOptions.getLayout()).numColumns,
					1).applyTo(this.buttonAutoAlignCols);
			this.buttonAutoAlignCols.setText("Redimensionare automata a coloanelor");
			WidgetCursorUtil.addHandCursorListener(this.buttonAutoAlignCols);
		...
		public final void populateFields() {
			...
			if (this.comboExtension.indexOf(ExportXlsPrefs.getExtension()) != -1) {
				this.comboExtension.select(this.comboExtension.indexOf(ExportXlsPrefs.getExtension()));
			} else {
				this.comboExtension.select(0);
			}
			...
			this.buttonAutoAlignCols.setSelection(ExportXlsPrefs.isAutoResizeCols());
			this.buttonShowHeader.setSelection(ExportXlsPrefs.isShowingHeader());
			ExportXlsOptionsView.this.buttonExportPathAuto.setSelection(ExportXlsPrefs.isUsingAutoExportPath());
			if (ExportXlsOptionsView.this.buttonExportPathAuto.getSelection()) {
				this.dsc.setDirPath(EncodeLive.getReportsDir());
			} else {
				this.dsc.setDirPath(ExportXlsPrefs.getExportPath());
			}
        ...
        public final void save() throws ConfigurationException {
			...
			if (!ExportXlsOptionsView.this.buttonExportPathAuto.getSelection()) {
				ExportXlsPrefs.put(ExportXlsPrefs.XLS_EXPORT_PATH, this.dsc.getSelectedDirPath());
			}
			ExportXlsPrefs.putBoolean(ExportXlsPrefs.XLS_EXPORT_PATH_AUTO,
					ExportXlsOptionsView.this.buttonExportPathAuto.getSelection());
			...
			ExportXlsPrefs.putBoolean(ExportXlsPrefs.XLS_AUTO_RESIZE_COLS,
					this.buttonAutoAlignCols.getSelection());
			ExportXlsPrefs.put(ExportXlsPrefs.XLS_EXPORT_EXTENSION, this.comboExtension.getText());
			ExportXlsOptionsView.this.settings.setSheetName(this.textSheetName.getText());
			ExportXlsPrefs.putBoolean(ExportXlsPrefs.XLS_IS_SHOWING_HEADER,
		...
		public final boolean validate() {
			if (this.comboExtension.getText().intern() == ExportXls.XLSX_EXTENSION) {
			...
			if (this.buttonAutoAlignCols.getSelection()
