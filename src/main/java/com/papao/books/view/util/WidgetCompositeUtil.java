...
public final class WidgetCompositeUtil {
    ...
    public static void centerInDisplay(final Composite parent) {
        try {
            ...
            int width = Display.getDefault().getPrimaryMonitor().getBounds().width;
            if (width > parentBounds.x) {
                width -= parentBounds.x;
            }
            int height = Display.getDefault().getPrimaryMonitor().getBounds().height;
            if (height > parentBounds.y) {
                height -= parentBounds.y;
            }
            parent.setLocation(width / 2, height / 2);
        } catch (Exception exc) {
    ...
    /**
     * @param parent suportul pe care se vor cauta in mod recursiv componentele Daca acesta nu are setata o imagine, ca sa obtinem ImageData din aceasta, se va crea o noua imagine, in gradient, folosind
     *               c1 si c2, iar toti descendentii suportului parent vor mosteni aceasta imagine, indiferent de nivel
     * @param c1     culoare nr 1 - afisata in partea stanga
     * @param c2     culoare nr 2 - afisata in partea dreapta Atentie! Daca avem 2 shell-uri, shell1 si shell2, iar shell2 = new Shell(shell1, style), din motive necunoscute, shell1.getChildren() are
